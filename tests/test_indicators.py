...
def test_curriculum_data():
    ...
    assert len(indicators_for("วิทยาศาสตร์และเทคโนโลยี", "ป.6")) == 25, "วิทยาศาสตร์ ป.6 ต้องมี 25"
    assert len([x for x in indicators_for("วิทยาศาสตร์และเทคโนโลยี", "ป.6") if x["std"] == "ว 8.1"]) == 6
    assert indicators_for("สังคมศึกษา ศาสนา และวัฒนธรรม", "ป.6") == []   # ยังไม่ได้เพิ่ม
    print("[ok] ชุดข้อมูลตัวชี้วัด: ภาษาไทย(34) + คณิต(31) + วิทยาศาสตร์(ป.6=25, ว8.1 ครบ 6)")
