...
def _sign_slots_h(n=3) -> str:
    """ช่องลงชื่อกรรมการตรวจรับ แนวราบ เลขอารบิก จุดไข่ปลายาว 30 จุด/ช่อง (ตามไฟล์ตัวอย่างเป๊ะ)"""
    dots = "." * 30
...
def _inspect_table(doc, menus, committee=None):
    ...
    widths = [Cm(2.7), Cm(4.5), Cm(1.99), Cm(7.31)]   # ตามไฟล์ตัวอย่างเป๊ะ
...
def render_order_doc(rnd, school, doc=None) -> str:
    ...
    left_lines = [f"ผู้รับจ้าง  {vname}", f"ที่อยู่ {vaddr or _BLANK}",
                  f"โทรศัพท์ {vphone or '-'}", f"เลขประจำตัวผู้เสียภาษี {vtax or '-'}"]
    right_lines = [f"ใบสั่งจ้าง เลขที่ {order_no}", f"วันที่ {_dnum(order_dt)}", f"{sname}", f"{saddr}"]
    htbl = doc.add_table(rows=1, cols=2)          # หัวแบบไร้เส้น (ตามแบบจริง)
    _no_borders(htbl); htbl.autofit = False
    htbl.rows[0].cells[0].width = Cm(9.0); htbl.rows[0].cells[1].width = Cm(6.5)
    _cell_lines(htbl.rows[0].cells[0], left_lines, size=14)
    _cell_lines(htbl.rows[0].cells[1], right_lines, size=14)
    ...
    if is_person:
        _p(doc, f"1. กำหนดส่งมอบภายใน {days} วัน นับถัดจากวันลงนามใบสั่งจ้าง", align="justify", indent=1)
        _p(doc, f"2. ครบกำหนดส่งมอบวันที่ {de}", indent=1)
    else:
    ...
    if is_person:
        _p(doc, "5. สงวนสิทธิ์ค่าปรับกรณีส่งมอบเกินกำหนด โดยคิดค่าปรับเป็นรายวันในอัตราร้อยละ 0.1 "
                "ของมูลค่าของตามใบสั่งจ้าง แต่จะต้องไม่ต่ำกว่าวันละ 100 บาท", align="justify", indent=1)
    else:
    ...
    if is_person:
        # โหมดจ้างบุคคล (แม่ครัว) ไม่แบ่งงวดงาน จึงต้องไม่มีคำว่า "งวด" ในเงื่อนไขข้อนี้
        _p(doc, f"8. การส่งมอบงานและการจ่ายเงิน ผู้รับจ้างต้องสรุปรายการประกอบอาหารให้แก่ผู้ว่าจ้าง "
                f"ภายใน {days} วัน นับถัดจากวันลงนามในใบสั่งจ้าง เพื่อทำการเบิกจ่ายเงิน",
           align="justify", indent=1)
    else:
    ...
    # ชื่อประเภทงานจ้างต้องตรงกับโหมด ไม่งั้นจะขัดกับตารางรายการด้านบนในเอกสารฉบับเดียวกัน
    work_name = ("การจ้างบุคคลประกอบอาหารกลางวัน" if is_person
                 else "การจ้างเหมาประกอบอาหารกลางวัน (ปรุงสำเร็จ)")
    _p(doc, f"10.1 ขอบเขตของงาน {work_name} ประจำภาคเรียนที่ {term} "
            f"ปีการศึกษา {prog.year}{'' if is_person else f' รอบ {rnd.seq}'} "
            f"(ระหว่างวันที่ {ds} ถึงวันที่ {de} จำนวน {days} วัน)",
       align="justify", indent=1.5, after=0)
