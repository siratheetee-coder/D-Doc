...
def render_hire_report_doc(rnd, school, doc=None) -> str:
    ...
    _p(doc, "โดยให้คณะกรรมการตรวจรับพัสดุ/ผู้ตรวจรับพัสดุที่ได้รับการแต่งตั้ง ปฏิบัติหน้าที่ตามระเบียบ"
...
def render_tor_doc(rnd, school, doc=None) -> str:
    ...
    period = f"ประจำวันที่ {_dnum(rnd.start_date)} ถึงวันที่ {_dnum(rnd.end_date)}"
    ...
    _p(doc, "มาตรฐานการศึกษาขั้นพื้นฐาน มาตรฐานที่ 1 ได้ให้ความสำคัญกับการสร้างให้ผู้เรียนมีสุขนิสัย "
            "สุขภาพกาย และสุขภาพจิตที่ดี การมีสุขภาพที่ดี สมบูรณ์ แข็งแรงเป็นสิ่งสำคัญที่จะทำให้ผู้เรียน "
            "สามารถมีพัฒนาการด้านต่าง ๆ ได้เหมาะสมตามวัยและเต็มตามศักยภาพ การให้นักเรียนได้รับประทาน "
    ...
    _p(doc, "3.1 ผู้เสนอราคาต้องเป็นบุคคลธรรมดาหรือนิติบุคคล มีอาชีพรับจ้างงานตามที่จัดจ้างในครั้งนี้",
       align="justify", indent=1, after=0)
    _p(doc, "3.2 ผู้เสนอราคาต้องไม่เป็นผู้ถูกระบุชื่อไว้ในบัญชีรายชื่อผู้ทิ้งงานของทางราชการ",
       align="justify", indent=1, after=0)
    _p(doc, "3.3 ผู้เสนอราคาต้องไม่เป็นผู้มีผลประโยชน์ร่วมกันกับผู้เสนอราคารายอื่น และไม่เป็นผู้กระทำการ "
            "อันเป็นการขัดขวางการแข่งขันราคาอย่างเป็นธรรม", align="justify", indent=1, after=0)
    _p(doc, "3.4 ผู้เสนอราคาต้องมีความสามารถตามกฎหมาย ไม่เป็นบุคคลล้มละลาย ไม่อยู่ระหว่างเลิกกิจการ "
            "และไม่เป็นบุคคลซึ่งอยู่ระหว่างถูกระงับการยื่นข้อเสนอหรือทำสัญญากับหน่วยงานของรัฐ",
       align="justify", indent=1)
    _p(doc, "4. ขอบเขตการดำเนินงาน", bold=True, indent=0.5, after=0)
    _p(doc, f"ผู้รับจ้างต้องเป็นผู้รับผิดชอบการประกอบอาหารกลางวัน (ปรุงสำเร็จ) {period} ให้แก่นักเรียน "
            f"ภายในวงเงินไม่เกิน {_money(total)} บาท ({bahttext(total)}) โดยผู้รับจ้างต้องดำเนินการ "
            "ประกอบอาหารกลางวันในทุกวันทำการ ตามรายการอาหารที่โรงเรียนกำหนด ทั้งนี้ คณะกรรมการตรวจรับพัสดุ "
            "หรือผู้ตรวจรับพัสดุจะเป็นผู้แจ้งรายการอาหารให้ผู้รับจ้างทราบก่อนประกอบอาหารอย่างน้อย 5 วันทำการ",
       align="justify", indent=1)
    _p(doc, "5. การส่งมอบพัสดุ", bold=True, indent=0.5, after=0)
    _p(doc, f"โรงเรียนกำหนดการส่งมอบงานออกเป็นจำนวน {len(insts) or '.......'} งวด โดยผู้รับจ้างต้องสรุป "
            "รายการประกอบอาหารเป็นรายวันภายในงวดงานนั้น ๆ ส่งให้แก่โรงเรียนเพื่อทำการเบิกจ่ายเงิน",
       align="justify", indent=1)
    ...
    _p(doc, f"เป็นเงินทั้งสิ้น {_money(total)} บาท ({bahttext(total)}) โดยมีแหล่งที่มาจาก{fund}",
       align="justify", indent=1)
    _p(doc, "8. งวดงานและการจ่ายเงิน", bold=True, indent=0.5, after=0)
    if insts:
        for it in insts:
            _p(doc, f"งวดที่ {it.seq} จ่ายเป็นเงิน {_money(it.amount or 0)} บาท เมื่อผู้รับจ้างสรุปรายการ "
                    f"ประกอบอาหาร {_dnum(it.start_date)} ถึง {_dnum(it.end_date)} และผ่านการตรวจรับแล้ว",
               align="justify", indent=1, after=0)
    else:
        _p(doc, "โรงเรียนกำหนดการจ่ายเงินเป็นงวดตามการส่งมอบและผ่านการตรวจรับของคณะกรรมการตรวจรับพัสดุ",
           align="justify", indent=1)
    _p(doc, "9. ค่าปรับ", bold=True, indent=0.5, before=2, after=0)
    _p(doc, "กำหนดค่าปรับอัตราร้อยละ 0.20 ของราคาค่าจ้างต่อวัน อ้างอิงตามหนังสือคณะกรรมการวินิจฉัยปัญหา "
            "การจัดซื้อจัดจ้างและการบริหารพัสดุภาครัฐ ที่ กค (กวจ) 0405.2/ว 116 ลงวันที่ 12 มีนาคม 2562",
       align="justify", indent=1)
