...
def _fill_cell(cell, lines, *, size=14):
    """เติมหลายบรรทัดในเซลล์ (ฟอนต์ไทย) รองรับบรรทัดหลาย run + ขีดเส้นใต้แบบจุด
    lines = [(text, align, bold)  หรือ  {'align':.., 'runs':[(text, bold, dotted), ...]}]"""
    cell.text = ""
    for i, ln in enumerate(lines):
        ...
        if isinstance(ln, dict):
            p.alignment = _ALIGN[ln.get("align", "left")]
            segs = ln["runs"]
        else:
            text, align, bold = ln
            p.alignment = _ALIGN[align]
            segs = [(text, bold, False)]
        for text, bold, dotted in segs:
            r = p.add_run(text)
            r.font.name = THAI_FONT
            _csize(r, size)
            _bcs(r, bold)
            if dotted:
                r.font.underline = WD_UNDERLINE.DOTTED
            r._element.rPr.rFonts.set(qn("w:cs"), THAI_FONT)
...
def render_w804(proc, school) -> str:
    ...
    pay = (ex.get("pay_source") or "").strip()
    if pay == "เงินยืม":
        pay_txt = f"ได้ใช้เงินยืม ตามสัญญา เลขที่ {_x(ex, 'loan_no')} ลงวันที่ {_xdate(ex, 'loan_date')}"
    elif pay == "เงินสดสำรองจ่าย":
        pay_txt = "ได้ใช้เงินสดสำรองจ่าย"
    else:   # ไม่ได้เลือก -> คงรูปเดิม (เว้นจุดให้เติมมือทั้งสองแบบ)
        pay_txt = f"ได้ใช้เงินยืม{_BLANK_S} ตามสัญญา เลขที่{_BLANK_S} ลงวันที่{_BLANK_S} / ใช้เงินสดสำรองจ่าย"
    _p(doc, "การจัดซื้อคราวนี้ไม่เกินวงเงินที่ประมาณไว้ และเห็นว่าเป็นราคาที่เหมาะสม และ"
            f"{pay_txt} "
            f"ชำระให้ผู้ขายเรียบร้อยแล้ว ตามหลักฐานการรับเงิน ใบเสร็จรับเงิน "
...
def _w119_box(doc, *, responsible, director_line, total_txt, baht_txt, advance_payer, fin,
    ...
    ซ้ายบน=ผู้รับผิดชอบ · ขวาบน=การเงิน · ซ้ายล่าง=แผน/พัสดุ(ติ๊กงบ) · ขวาล่าง=ผอ."""
    def CHK(on): return "☑" if on else "☐"
    def RAD(on): return "●" if on else "○"
    plan = budget_kind in ("รายหัว", "15 ปี", "ตามแผน")
    ...
    _fill_cell(t.rows[0].cells[1], [
        ("ความเห็นของงานการเงิน", "left", True),
        (f"เรียน  {director_line}", "left", False),
    ] + (
        # ตาราง 2 ไม่ใช่การจัดซื้อจัดจ้าง -> ขอแค่อนุมัติจ่ายเงิน บรรทัดเดียว ไม่ต้องแยกข้อ
        # (ประหยัดพื้นที่ให้เอกสารจบหน้าเดียว ลายเซ็น ผอ. ไม่หลุดไปหน้า 2)
        [(f"โปรดพิจารณา อนุมัติให้จ่ายเงิน จำนวน {total_txt} บาท", "left", False)]
        if table2 else
        [("โปรดพิจารณา", "left", False),
         ("๑. ให้ความเห็นชอบการจัดซื้อจัดจ้างดังกล่าวข้างต้น", "left", False),
         (f"๒. อนุมัติให้จ่ายเงิน จำนวน {total_txt} บาท", "left", False)]
    ) + [
        (f"( {baht_txt} )", "left", False),
        (f"ให้แก่ {advance_payer} ผู้ทดรองจ่าย/ผู้ยืมเงิน", "left", False),
        ("(ลงชื่อ).............................. เจ้าหน้าที่การเงิน", "left", False),
        (f"( {fin} )", "left", False),
    ])
    # บรรทัดงบ: ส่วน "อื่นๆ" ตามด้วยรายละเอียดที่ขีดเส้นใต้แบบจุด (ว่าง = เส้นจุดให้เติมมือ)
    other_seg = ("  " + budget_other + "  ") if budget_other else "                              "
    _fill_cell(t.rows[1].cells[0], [
        ("ความเห็นของงานแผนงานและงานพัสดุ", "left", True),
        (f"{CHK(plan)} ใช้งบตามกิจกรรม / โครงการที่อนุมัติไว้ตามแผน", "left", False),
        {"align": "left", "runs": [
            (f"     งบ  {RAD(budget_kind=='รายหัว')} รายหัว   {RAD(budget_kind=='15 ปี')} 15 ปี   "
             f"{CHK(budget_kind=='อื่นๆ')} อื่นๆ", False, False),
            (other_seg, False, True),
        ]},
        ("", "left", False),
        ("(ลงชื่อ)..............................เจ้าหน้าที่พัสดุ", "left", False),
    ...
    _fill_cell(t.rows[1].cells[1], [
        ...
        ("", "left", False),
        ("(ลงชื่อ) ..............................", "center", False),
        (f"( {director} )", "center", False),
        (director_office, "center", False),
        ("วันที่ ..............................", "center", False),
    ])
    return t
...
def render_w119_t1(proc, school) -> str:
    ...
    _w119_box(doc,
              ...
              budget_kind=(ex.get("budget_kind") or "").strip(),
              budget_other=(ex.get("budget_other") or "").strip(),
              director=(getattr(school, "director_name", "") or "").strip() or _BLANK,
              director_office=_director_office(school))
    return _save(doc, f"ว119ตาราง1_{(proc.memo_no or proc.id)}_{(proc.subject or '').strip()}")


# รายการค่าใช้จ่ายตามตารางที่ 2 ของ ว.119 (ไม่ถือเป็นการจัดซื้อจัดจ้าง)
# อ้างตามตัวหนังสือ ว.119 ตาราง 2 - ใช้เป็นตัวเลือกช่วยจำ ไม่ใช่การจำกัดรายการ
W119_T2_ITEMS = [
    "ค่าอาหารว่างและเครื่องดื่ม / ค่าอาหาร (กรณีประชุมคาบเกี่ยวมื้ออาหาร)",
    "ค่าอาหารว่างและเครื่องดื่ม (กรณีหน่วยงานอื่นเข้าดูงาน)",
    ...
    "ค่าผ่านทางด่วนพิเศษ",
    "ค่าพาหนะ (กรณีได้รับมอบหมายให้เดินทางไปปฏิบัติราชการ)",
...
def render_w119_t2(proc, school) -> str:
    """ว.119 ตารางที่ 2 - ค่าใช้จ่ายที่ "ไม่ถือเป็นการจัดซื้อจัดจ้าง"

    ต่างจากตารางที่ 1 อย่างสำคัญ
      - ตาราง 1 = เป็นการจัดซื้อจัดจ้าง แต่วงเงินไม่เกิน 10,000 บาท ทำไปก่อนแล้ว
        "รายงานขอความเห็นชอบ" ภายใน 5 วันทำการ และถือรายงานเป็นการตรวจรับโดยอนุโลม
      - ตาราง 2 = ไม่ใช่การจัดซื้อจัดจ้างตาม พ.ร.บ.ฯ จึง **ไม่ต้องทำรายงานขอความเห็นชอบ
        และไม่มีเพดาน 10,000 บาท** ให้เบิกจ่ายตามระเบียบการเงิน/ระเบียบค่าใช้จ่ายเฉพาะเรื่อง
    เอกสารที่ออกตรงนี้จึงเป็น "บันทึกขออนุมัติเบิกจ่าย" ไม่ใช่รายงานขอความเห็นชอบ
    """
    doc = _font_doc()
    total = _total(proc)
    ...
    _header(doc, subject_line="ขออนุมัติเบิกจ่ายเงิน",
            school=school, doc_no=(proc.memo_no or "").strip(), doc_date=proc.request_date)
    _p(doc, f"ด้วย {dept} ได้ดำเนินกิจกรรม{(proc.subject or '').strip()} "
            f"จำนวน {n} รายการ รวมเป็นจำนวนเงินทั้งสิ้น {_money(total)} บาท ({bahttext(total)}) "
            f"โดยมีหลักฐานการจ่ายเป็น ใบส่งของ/ใบแจ้งหนี้/ใบเสร็จรับเงิน/ใบสำคัญรับเงิน "
       ...
       align="justify", indent=1.25, after=2)
    _p(doc, f"ค่าใช้จ่ายรายการนี้เป็นค่าใช้จ่ายตามตารางที่ 2 แนบท้าย{_W119_REF} "
            "ซึ่งไม่ถือเป็นการจัดซื้อจัดจ้างตามพระราชบัญญัติการจัดซื้อจัดจ้างและการบริหารพัสดุภาครัฐ "
            "พ.ศ. 2560 จึงไม่ต้องจัดทำรายงานขอความเห็นชอบตามตารางที่ 1 และไม่มีเพดานวงเงิน "
            "โดยให้เบิกจ่ายตามระเบียบกระทรวงการคลังว่าด้วยการเบิกค่าใช้จ่ายในการบริหารงานของส่วนราชการ "
            "หรือระเบียบค่าใช้จ่ายเฉพาะเรื่องที่เกี่ยวข้อง แล้วแต่กรณี",
       align="justify", indent=1.25, after=2)
    ...
    _w119_box(doc,
              ...
              budget_other=(ex.get("budget_other") or "").strip(),
              director=(getattr(school, "director_name", "") or "").strip() or _BLANK,
              director_office=_director_office(school), table2=True)
