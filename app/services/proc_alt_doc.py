...
def render_w804(proc, school) -> str:
    ...
    _p(doc, "การจัดซื้อคราวนี้ไม่เกินวงเงินที่ประมาณไว้ และเห็นว่าเป็นราคาที่เหมาะสม และได้ใช้เงินยืม"
            f"{_BLANK_S} ตามสัญญา เลขที่{_BLANK_S} ลงวันที่{_BLANK_S} / ใช้เงินสดสำรองจ่าย "
...
def _w119_box_t1(doc, *, officer, director_line, total_txt, baht_txt, advance_payer, fin,
                 budget_kind, budget_other, director, director_office):
    """กล่องความเห็น ว.119 ตาราง 1 — ตาราง 2×2 มีเส้นขอบ (ตาม template) · ฟอนต์ 14
...
def render_w119_t2(proc, school) -> str:
    ...
    director = (getattr(school, "director_name", "") or "").strip() or _BLANK
    _box = doc.add_table(rows=1, cols=2)
    _box.style = "Table Grid"
    _box.autofit = False
    _box.rows[0].cells[0].width = Cm(8.1)
    _box.rows[0].cells[1].width = Cm(8.1)
    _fill_cell(_box.rows[0].cells[0], [
        ("ความเห็นหัวหน้างานการเงิน", "left", True),
        ("☐ เห็นสมควรตามที่เสนอ", "left", False),
        ("☐ ไม่เห็นสมควร เนื่องจาก..............................", "left", False),
        ("", "left", False),
        ("ลงชื่อ................................หัวหน้างานการเงิน", "left", False),
        (f"( {fin_head or _BLANK} )", "left", False),
    ])
    _fill_cell(_box.rows[0].cells[1], [
        ("ผู้มีอำนาจลงนาม", "left", True),
        ("☐ เห็นชอบ/อนุมัติ", "left", False),
        ("☐ ไม่เห็นชอบ/ไม่อนุมัติ", "left", False),
        ("", "left", False),
        ("ลงชื่อ.........................................", "left", False),
        (f"( {director} )", "left", False),
        (_director_office(school), "left", False),
        ("วันที่.........................................", "left", False),
    ])
