...
_PROJECT_PROMPT = """คุณคือผู้ช่วยครูไทย อ่าน "เอกสารโครงการ" ของโรงเรียนด้านล่าง แล้วดึงข้อมูลไปเติมแบบรายงานผลการดำเนินงานโครงการ
ตอบกลับเป็น JSON เท่านั้น (ไม่มีคำอธิบายอื่น) ตามคีย์นี้:
{
 "title": "ชื่อโครงการ/กิจกรรม",
 "location": "สถานที่ดำเนินการ",
 "responsible": "ชื่อผู้รับผิดชอบโครงการ (คนแรก)",
 "std_ref": "สนองมาตรฐาน/กลยุทธ์/นโยบาย (ข้อความตามต้นฉบับ)",
 "principles": "หลักการและเหตุผล/ความเป็นมา (ย่อหน้าตามต้นฉบับ)",
 "objectives": ["วัตถุประสงค์ข้อละ 1 รายการ"],
 "target_qty": ["เป้าหมายเชิงปริมาณ"],
 "target_qual": ["เป้าหมายเชิงคุณภาพ"],
 "expected": ["ผลที่คาดว่าจะได้รับ"],
 "steps_items": [{"act": "กิจกรรม/ขั้นตอน", "period": "ระยะเวลา", "who": "ผู้รับผิดชอบ"}],
 "eval_items": [{"indicator": "ตัวชี้วัดความสำเร็จ", "method": "วิธีการประเมิน", "tool": "เครื่องมือ"}],
 "budget_planned": งบประมาณรวมเป็นตัวเลข (ไม่มีจุลภาค, ไม่พบใส่ 0)
}
- คงข้อความตามต้นฉบับ ห้ามแต่งข้อมูลที่ไม่มีในเอกสาร ส่วนที่ไม่พบให้ใส่ "" หรือ []
- ตัดเลขข้อนำหน้า (1. 2.) ออกจากรายการ ใช้เลขอารบิก
...
def read_project_doc(text: str, api_key: str) -> dict:
    """สกัดข้อมูลโครงการ (จากไฟล์เอกสารโครงการที่อัปโหลด) ไปเติมแบบรายงานผลโครงการ"""
    text = (text or "").strip()
    if len(text) < 80:
        return {"error": "no_text"}
    return _ai_write_json(_PROJECT_PROMPT.replace("{TEXT}", text[:24000]), api_key)
