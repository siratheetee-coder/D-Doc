...
SELLER = {
    ...
    # โปรลดเฉพาะราคา bundle (ซื้อครบทุกงานเท่านั้น) — ราคางานแยกและบันไดส่วนลด
    # (2 งาน -100 / 3 งาน -200 / 4 งาน -300) คงเดิมทุกอย่าง จึงไม่ต้องระบุ p_* ที่นี่
    # ไม่ระบุคีย์ไหน = ใช้ราคาปกติจาก REGULAR_PRICES
    "promo": {
        "enabled": False,                     # เปิดโปร -> True
        "start": "",                          # วันเริ่มนับ เช่น "2026-07-20" (โปร 7 วันจากวันนี้)
        "days": 7,
        "slots": 30,                          # จำกัดกี่สิทธิ์แรก (แสดงเป็นข้อความ)
        "bundle": 1990,                       # ครบทุกงาน 5 งานช่วงโปร (จากปกติ 2,190 -> ประหยัดเพิ่ม 200)
    },
