...
## 3.6) ลบข้อมูลโรงเรียนที่ไม่มีการใช้งาน (15 เดือน)  ★ ติดตั้งพร้อม setup-cron.sh

`setup-cron.sh` ตั้งให้รันทุกวันตี 3:30 อยู่แล้ว · **ก่อนใช้จริงให้ลองดูก่อนว่าจะเกิดอะไรขึ้น:**
```bash
cd /opt/ddoc && ./.venv/bin/python -m app.services.retention --dry-run
```
โหมดนี้ไม่แตะข้อมูลและไม่ส่งอีเมล แค่บอกว่าจะเตือนใคร/จะลบอะไร

กำหนดการ: เตือนทางอีเมลเมื่อไม่ใช้งานครบ **12 / 13 / 14 เดือน** แล้ว **ลบเมื่อครบ 15 เดือน**

กติกาความปลอดภัยที่ใส่ไว้:
- **โรงเรียนที่ยังเป็นสมาชิกและไม่หมดอายุ ไม่ลบเด็ดขาด**
- **ลบได้เฉพาะที่เตือนครบ 3 ครั้งแล้วจริง** (บันทึกใน DB) — วันแรกที่เปิดใช้ฟีเจอร์นี้จึงลบอะไรไม่ได้เลย
- เตือนสองครั้งต้องห่างกัน ≥ 25 วัน · ลบได้ไม่เกิน 20 โรงเรียนต่อรอบ
- ทุกการเตือน/ลบบันทึกลง audit log

ดูโรงเรียนที่ใกล้ถูกลบได้ในคอนโซลผู้ดูแลระบบ (ขึ้นป้ายสีส้ม/แดงในการ์ดของโรงเรียนนั้น)
