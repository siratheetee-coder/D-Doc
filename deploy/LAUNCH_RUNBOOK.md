...
- proxy-headers ในไฟล์ service (ข้อ 2)
- สำรองข้อมูล: เข้ารหัส AES-256 + ส่งขึ้นคลาวด์ + `restore.sh` (ข้อ 3)
- health check อัตโนมัติ + เตือนดิสก์ใกล้เต็ม (ข้อ 3.5)
- ลบข้อมูลที่ไม่ใช้งาน 15 เดือน + เตือน 3 ครั้ง (ข้อ 3.6)
- พร้อมรันหลาย worker (ข้อ 4.5) - ค่าเริ่มต้นยังเป็น 1
- PDPA: เลิกเก็บข้อมูลอ่อนไหวของนักเรียน (เลขบัตร/ที่อยู่/สุขภาพ/รูปถ่าย)
- audit log บันทึกเหตุการณ์สำคัญ + หน้า /audit สำหรับไอดีหลัก
- ยืนยันตัวตน 2 ชั้น (TOTP) ทุกบัญชี · ไม่บังคับ · มีปุ่มปลดล็อกให้ผู้ดูแล
- /privacy เติมเนื้อหาจริงแล้ว (รอทนายตรวจ)

## ยังเหลือ (ไม่ใช่งานโค้ด - เจ้าของทำ)
1. **จดทะเบียนบริษัทจำกัด** - ตอนนี้รับผิดด้วยทรัพย์สินส่วนตัวไม่จำกัด
2. **ToS + ข้อตกลงประมวลผลข้อมูล (DPA)** ให้ทนายร่าง + จำกัดความรับผิด
   (PDPA ม.40 บังคับให้ผู้ควบคุมข้อมูลต้องมี DPA กับผู้ประมวลผล)
3. ตั้ง `BACKUP_S3_*` ให้ไฟล์สำรองออกนอกเครื่อง (ข้อ 3.2)
4. ซ้อมกู้คืนจริงอย่างน้อย 1 ครั้ง แล้วทำซ้ำทุก 3 เดือน (ข้อ 3.4)
5. เปิด 2FA ให้บัญชีผู้ดูแลระบบของตัวเอง + เก็บรหัสสำรองนอกเครื่อง
6. ทดสอบจ่ายเงินจริง end-to-end (ข้อ 6)
7. ประกันภัยไซเบอร์ (ถ้ามีโรงเรียนเยอะขึ้น)
